...
object LDPCmdTst {
	...
	val BLAcl = w3c(URI("/People/Berners-Lee/.acl#TimRl").a(wac.Authorization)
			-- wac.agent ->- (URI("/People/Berners-Lee/card#i") -- wac.accessToClass ->- (bnode() 
				-- containedIn ->- URI("/People/Berners-Lee/")
			)
		)) union w3c(
	...
	val rootACL = w3c(URI("/.acl#Admin").a(wac.Authorization)
		-- wac.mode ->- wac.Control
		-- wac.agentClass ->- URI("/Admins#g")
		-- wac.accessToClass ->- bnode("allContents")).graph union
		w3c(URI("/.acl#Public").a(wac.Authorization)
			-- wac.accessTo ->- URI("/")
			-- wac.mode ->- wac.Read
	...
	def w3cu(u: String): Uri = pod.withPath(Uri.Path(u))
